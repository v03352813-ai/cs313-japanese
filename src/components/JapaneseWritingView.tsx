...
export const JapaneseWritingView: React.FC<JapaneseWritingViewProps> = ({ isVip, onOpenVipModal }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      {/* 顶部标题区 */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-sky-600 via-indigo-600 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-lg relative overflow-hidden">
        <div className="space-y-1.5 z-10">
          ...
          <p className="text-xs text-sky-100 font-medium max-w-2xl leading-relaxed">
            告别中式日文与文体混用！涵盖商务书信、EJU留考论述、图表分析与日常随笔，配备<strong>日本官方400字詰原稿用紙模式</strong>与<strong>AI文体/敬语四维诊断精批</strong>。
          </p>
        </div>

        <div className="flex items-center gap-2 self-start md:self-auto shrink-0 z-10">
          <button
            onClick={() => {
              setUserInputText('');
              setEvalResult(null);
